...
### Accessing All Reader Connections

For advanced use cases like SharedResource sharing between domains:

```php
// Get count without creating connections (lazy-friendly)
$count = $pool->getReaderCount(); // Returns number of configured readers

// Get all reader connections
$allReaders = $pool->getReaders(); // Returns array of DbConnectionInterface

// Share PDO connections with other domains
foreach ($allReaders as $reader) {
    $pdo = $reader->pdo();
    // Share $pdo with other domain...
}
```
...
DbConnection (Factory)
    ...
    └─→ External extends PdoConnection (wraps existing PDO)
```
